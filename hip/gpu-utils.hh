...
{
#ifdef __HIPCC__
  HIPCHECK(hipMalloc((void**)dst, sizeof(T) * nelms));
  HIPCHECK(hipMemcpy(*dst, src, sizeof(T) * nelms, hipMemcpyHostToDevice));
#else
...
{
#ifdef __HIPCC__
  HIPCHECK(hipMemcpy(dst, src, sizeof(T) * nelms, hipMemcpyDeviceToHost));
#else
  memcpy(dst, src, sizeof(T) * nelms);
#endif
}
...
{
#ifdef __HIPCC__
  HIPCHECK(hipMemcpy(d_array, h_array, sizeof(T) * n, hipMemcpyHostToDevice));
#else
...
{
#ifdef __HIPCC__
  HIPCHECK(hipMemcpy(h_array, d_array, sizeof(T) * n, hipMemcpyDeviceToHost));
#else
